 ...
 * 无从查起。分列之后它重新出现在全局那一栏里，所以**必须同时贴 `ShadowTag`**，否则界面等于宣称一条
 * 不生效的内容在生效。
 *
 * **内置层不出现。** 它随程序发布、只读、用户看不见，给它画一个标签页等于画一个点了没反应的按钮。
 *
 * **这里只管「看哪一层 / 加到哪一层」。** 不要往这一条里塞逐条启停：层是这条内容住在哪个目录，启停
 * 是某一轮用不用它，两件事的生效范围不同，合到一个控件上，取消勾选读起来就是从盘上删掉。
 */
 ...
 *
 * 这是**边界不是装饰**：不贴的话，全局那一栏里留有一条永远不会被用到的内容，
 * 而它看起来和生效的那些一模一样。
 */
