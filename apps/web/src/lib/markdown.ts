 ...
 *    已注册语言打分，模型每产出一个 token 就重跑一次，长代码块会直接卡死。
 *    所以增长中的尾段只高亮显式标了语言的块，定稿后再整段重渲染并开启自动检测。
 * 2. **必须净化。** 结果会原样进 innerHTML，而模型输出不可信——它读的可能是别人
 *    仓库里的 README，`<img onerror>` 这类注入是现实威胁。
 *
 * 3. **highlight.js 按需加载。** 它的公共语言包约 200 kB，同步 import 会让首屏包从
 *    44 kB 涨到 273 kB，而大多数会话的头几屏没有代码块。这里先用未高亮的结果渲染，
 *    后台把库拉回来后置位信号，由 Solid 的细粒度更新自动重渲染受影响的那几段——
 *    用户看到的是代码块「先出现、随后上色」，不是白屏等待。
 *    **读 `highlightReady()` 的地方不许套 `untrack`**，套了就建不起订阅，库到了
 *    也不会重渲染。
 *
 ...
 * 流式期的增量渲染。
 *
 * **边界由 lexer 划，不由文本扫描划；不要改回「按最后一个空行切」。** 逐字符差分测过，至少五类块跨
 * 空行：松散列表的紧凑/松散定型、有序列表续号、列表项内的四空格续行、含内部空行的缩进代码、HTML
 * 块。认全它们等于在扫描器里重写一遍块级语法，而块结构的权威本来就是 lexer。
 *
 * **为什么留两个 token 不定稿。** 增长中的末 token 会变型并**向前合并一格**：`1. 一
 ...
 *
 * **已定稿的 def 必须种副本进去。** marked 的引用式链接表**先到先得**
 * （`this.tokens.links[tag] || (...)`）。把状态里那张表直接交给 lexer 的话，尾块里半截的
 * `[spec]: htt` 会被当成合法 def 写进去并永久占位，之后补全的那条再也进不来。所以交副本，且只把**
 * 已定稿区**里的 def 记进状态。
 *
 * **已知偏差。** use 已定稿、def 隔两个块以上才到的前向引用，流式期保持字面文本。定稿时的整段重渲染
 * （`renderMarkdown`）纠正它——差别只存在于流式期。
 *
 * **最坏情形。** 整篇是一个松散列表时顶层只有两个 token，边界推不动，退化为每档重解析全文
...
export interface StreamRenderer {
  /** 传当前全文，拿这一档要落地的片段。**只接受追加**：文本变短视为换了一份，整份重来。 */
