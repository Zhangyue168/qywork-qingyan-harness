 ...
 * 正文流的匀速呈现。
 *
 * **为什么需要它。** `text.delta` 到达的节奏是网络的节奏，不是阅读的节奏。中转站按批转发是常态
 * （2026-08-20 实测一家：每约 960ms 给 37 字一批），直接写进 transcript 的话，
 * 画面就跟着网络一起一顿一顿。**成因不是渲染慢，是到达节奏本身不均匀。**
 *
 * 这里把「收到」和「显示」解耦：收到的先攒着，按固定 50ms 一档往外放。
 *
 * **每档放多少：三条规则各管一段。** `sliceSize` 是
 * `clamp(估计流速, 积压/(蓄水池×CATCHUP_RATIO), 积压/蓄水池)`。
 *
 ...
 *
 * **蓄水池按实测到达间隔定，不是常数。** 稳态落后就等于蓄水池的档数（积压涨到 `蓄水池×流速` 时上界
 * 才不再压着流速），而这份落后在终态会一次性倒出去——**蓄水池多存一档，结尾就多蹦一档的字**。存
 * 多少只由一件事决定：**撑不撑得到下一批**。所以它跟着 `gapHold`（近期到达间隔）走：批量转发的上游
 * 间隔 960ms，存满 10 档；逐 token 到达的上游间隔几十毫秒，存 2 档就够。2026-08-20 实测（400/1200
 * 字每秒的平顺上游）：固定 10 档落后 440ms、结尾倒出 162/486 字；跟着间隔走落后 134ms、结尾倒出
 * 34/101 字，而 37 字/960ms 的批量序列逐档不变。
 *
 * **卡顿不是流速。** 间隔超过 `STALL_MS` 的那一次**不更新速率估计**。上游真卡住六秒再恢复，
 * 若把这段算进流速，估计值会被拉到极低，之后的字是一个一个往外挤的。
 * 卡顿期间积压放完就停表——**卡住就该看起来是卡住**，不靠拖慢正文假装还在流。
 *
 * **终态一律不排队。** run 结束、报错、被中断，都要立刻把积压全放出去。原因不是好看：
 ...
 * 和真实定时器，因此只有纯函数那半有测试、编排这半没有——而真正会出错的是
 ...
 * 主线程被这个频率占满时，正文那个 50ms 定时器跟着被推迟，表现是正文有数据却不动。
 *
 * 用与正文同一个 `TICK_MS`：两个不同步的周期串起来会拍频（`reparseSkip` 那条讲的
 * 就是这件事），同一个节拍则永远落在同一拍上。
 *
 * 按 stepId 分桶而不是只留最后一条：一波并发工具会同时产出，混成一桶就串卡片了。
 */
