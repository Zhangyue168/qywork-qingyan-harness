 ...
 *
 * 覆盖时长实测（12 秒一轮的固定负载，直接读环里最老帧与最新帧的时刻差）：每轮
 * `run_command` 输出 1.8 MB 时环由这条字节上限封顶，窗口 264 秒；每轮输出 8 KB 时
 * 先撞 `RETAIN` 的 5000 帧，窗口约 56 分钟。
 */
...
export class EventBus {
   ...
   * 拆成两处的话，「落后了多少」和「这个落后是不是本流上的」就成了两个
   * 可以各自改坏的判断。
   */
