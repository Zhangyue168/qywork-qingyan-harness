 ...
 *
 * 它锁的是一组真实症状：切会话之后界面显示的是上一条会话的内容，偶尔卡死。
 * 根因不在「切换」那段代码里，在这里——**归属信息只用于发送那一刻的过滤，
 * 从不随帧发出**，而那层过滤在三处漏了：
 *
 * 1. 空订阅集被当成「全订阅」，因此前端发 `subscribe([])` 想退订，收到的是全部；
 * 2. 断线补发这条路上没有过滤，重连一次最多灌 5000 帧别人的事件；
 * 3. 客户端拿不到归属，无法自保。
 *
...
describe('订阅语义：null 和空集不是一回事', () => {
   ...
   * 因此所有会话的事件一起涌向这个客户端，而客户端无条件写进当前 transcript。
   */
...
describe('断线补发按订阅过滤', () => {
  ...
  /** 缺口超出保留窗口要明确回 null，让客户端改走全量重拉，而不是静默少几条。 */
  test('缺口超出保留窗口回 null', () => {
    const bus = new EventBus()
 ...
 * 补发零条、resync 为假——因此界面永远停在断线那一刻，那一轮一直显示执行中，
...
describe('换了一条流就不能按位置比大小', () => {
   ...
   * 「新流的 (lastSeq, now]」并宣称补全了，而被跳过的是新流开头那一段。
   */
