 ...
 * 2. **名字长得像凭证** —— 兜底。抓的是明文未知的那些 key（用户自己的
 *    `GITHUB_TOKEN`、CI 注入的那一批）。误伤率最高，所以给了 `allow` 出口。
 *
 * **最容易出事的地方：短值。** 按值匹配是 `value.includes(secret)`。如果某个 secret 明文是 `"1"`
 * 或空串（配置写错、`apiKey: ""`、占位符没删），那**整份环境**都会命中——命令拿到一个空环境，症
 * 状是所有命令都失败，而报错里没有任何指向脱敏模块的线索。所以低于 `MIN_SECRET_VALUE_LENGTH` 的值
 * 一律不参与按值匹配，只按名字剥。
 */
 ...
 *
 * 按明文只认得 `collectSecrets` 收到的那几个值——配置里的那几把 apiKey。
 * `cat ~/.ssh/id_rsa`、`cat .env` 的输出**一个字都不会被脱敏**，原样进上下文再随
 * 下一次请求发给 provider：那些明文本地未知，按值这条结构上就抓不到它们。
 *
 ...
 * 流式脱敏。
 *
 * **为什么不能逐块调 `redactSecrets`。** 命令输出是分片到达的，一个 key 完全可能**跨片**：`sk-abc`
 * 落在这一片、`def123…` 落在下一片。两片各自脱敏都不命中，拼起来就是完整明文——而且这种漏网只在
 * 输出恰好在那个位置断开时才发生，**本地几乎复现不出来**。
 *
 * 所以每片都留一段尾巴不发，长度取「最长 secret 减一」：任何跨界的 secret
 * 必然有一部分落在这段尾巴里，等下一片到了一起处理。留的字节数是常数级的
 * （一个 key 几十字符），代价可以忽略。
 *
 * **顺序很关键：先脱敏，再切尾巴。** 反过来（先按位置切、再对前半段脱敏）的话，一个恰好跨过切点
 * 的 secret **前半截会被当成「安全部分」原样发出去**。实测形状：逐字符喂一个 32 字符的 key，
 * 完整明文漏出。
 *
...
export function createStreamRedactor(secrets: SecretSet): {
   ...
   * 出现已知的那几个明文」时成立，而这里还有形状脱敏：
   * `cat ~/.ssh/id_rsa` 的私钥、`.env` 里的 token，**它们的明文本地从来不知道**，
   * 是这条链路唯一能抓到它们的地方。直通等于把这一层关掉。
   */
  ...
  return {
    push(chunk: string): string {
       ...
       *
       * **不要退回成只按字节扣**：`hold` 是 256 字节，而一条命令一秒输出一行七个字节，
       * 因此跑完之前一个字都交不出去——中途输出那条通道整个不成立，
