 ...
 * 该值保持不变、连接仍然建立，症状表现为前端出现 `undefined.id`。
...
export interface ServerCapabilities {
  ...
  // 前端通过 `isDesktopShell()` 判定是否为桌面壳。握手是服务端对客户端的声明，
  ...
  mode: PermissionMode
  /**
   * 内置浏览器。**两件事分开报，不合成一个布尔。**
   *
   * 合成之后「运行时版本不达标」会被读成「浏览器用不了」，而手动浏览此时照常可用。
   *
...
/**
 * 电脑控制此刻可用到什么程度。三项依次成立，工具注册要求三项同时为真。
 *
