...
`,
  ...
  },
  {
     ...
     * 有三种可选方案——
     *
     * 1. 换算成美元：要一个汇率。汇率持续变动，落盘的数值很快便不再准确，
     *    而它看起来仍然是个确切的金额。
...
`,
  },
  {
     ...
     * 只读，产出物是静态的，不承诺可回写。这里的归档是「不在列表里显示」，数据一条不动。两个都叫
     * archive 会持续制造误读，所以这里点名说清；导出那套保持原名。
     *
     * **数据不删除，但界面上无法访问。** `listConversations` 过滤它，`getConversation` **不过滤**——按
...
`,
  },
  {
     ...
     * 该理由不成立：它造出了第二条线——输入区的 chip 写会话、设置页的下拉写
     * `config.effort`，两处各写各的。表现是**在 chip 上选的档换个会话就没了**，
     * 而设置页中的值看似仍在，两处互不知晓对方已修改。
     *
...
`,
  },
  {
     ...
     * 那条 run 便会逃过回收——而它已经没有任何人在跑，会话就此锁死。
...
`,
  },
  {
     ...
     * 因此标题和目标写着一模一样的串，目标那一层将重复占用而不含信息。对象名应当填类名，
...
`,
  },
  {
     ...
     * `additionalProperties: false`，模型照历史仿造一个 `action`，会浪费一轮请求。
