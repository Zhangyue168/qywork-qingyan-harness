 ...
 * stderr——用户装完插件想知道「它加载了吗、被隔离了吗」，只能起一轮真实 run 然后去日志里翻，而桌面
 * 外壳会把那些输出吞掉。
 *
 * **为什么隔离状态要逐条打，而不是一句「已隔离」。** 两个维度的成立条件不同（版本要求不同，bun 上一
 * 个都没有），而且 `process:exec` 会架空出网闸。合成一句话的话，「已隔离」在不同机器上就不是一个意
 * 思了——那正是 ARCHITECTURE §17.1 纠正过的错误。
 *
